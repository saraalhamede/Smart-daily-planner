...
const categoryRules = [
  ...
  {
    category: "writing",
    words: ["write", "summary", "report", "presentation", "document"],
  },
  {
    category: "routine",
    words: ["meeting", "call", "email", "clean", "organize", "buy"],
  },
]; 
...
export function categorizeTask(task) {
  ...
  const difficulty = clamp(
    ...
    1,
    5,
    defaultDifficulty(category, text),
  );
...
function buildAdvice(predictedEnergy, stressLevel, isTired) {
  if (predictedEnergy <= 2)
    return "Low energy detected. Prefer easier tasks and shorter sessions.";
  if (stressLevel >= 4)
